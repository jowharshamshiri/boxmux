...
pub enum TerminalMessage {
    ...
    ProcessBytes(Vec<u8>),
    SendInput(String),
    ResizeTerminal { width: usize, height: usize },
    
    // Screen buffer operations
    SwitchScreenBuffer { to_alternate: bool },
    ClearScreen { mode: ClearMode },
    ClearLine { line: usize, mode: ClearMode },
    ...
    SetCursorColumn { col: usize },
    SetCursorLine { line: usize },
    CursorUp { lines: usize },
    CursorDown { lines: usize },
    CursorLeft { cols: usize },
    CursorRight { cols: usize },
    CursorToLineStart,
    CursorNextLine { lines: usize },
    CursorPreviousLine { lines: usize },
    
    // Terminal modes
    SetMode { mode: TerminalModeType, enabled: bool },
    
    // Character attributes and formatting
    SetAttributes { attributes: TerminalAttributes },
    ResetAttributes,
    
    // Scrolling regions
    SetScrollingRegion { top: usize, bottom: usize },
    ResetScrollingRegion,
    ScrollUpInRegion { lines: usize },      // F0307: Scroll up within margins
    ScrollDownInRegion { lines: usize },    // F0307: Scroll down within margins
    
    // Tab operations
    SetTabStop,
    ClearTabStop,
    ClearAllTabStops,
    TabForward { count: usize },
    TabBackward { count: usize },
    
    // Device control and queries
    DeviceStatusReport { report_type: DeviceReportType },
    DeviceAttributes { primary: bool },
    ...
    ClipboardOperation { operation: ClipboardOp },
    ...
    // Character sets
    SetCharacterSet { set: CharacterSetType, charset: String },
...
impl TerminalScreenBuffer {
        ...
        if y < self.height {
            for _ in 0..count {
                if self.content.len() > y {
                    self.content.insert(y, vec![TerminalCell::default(); self.width]);
    ...
    fn terminal_row_to_string(&self, row: &[TerminalCell]) -> String {
        ...
        for cell in row {
            ...
            if format_changed {
                result.push_str(&self.generate_ansi_formatting(
                    current_fg, current_bg, current_bold, current_underline,
                    current_italic, current_reverse, current_strikethrough
                ));
            }
        ...
        if current_fg.is_some() || current_bg.is_some() || current_bold || 
           current_underline || current_italic || current_reverse || current_strikethrough {
    ...
    fn generate_ansi_formatting(&self, fg: Option<u8>, bg: Option<u8>, 
                               bold: bool, underline: bool, italic: bool, 
                               reverse: bool, strikethrough: bool) -> String {
        ...
        if reverse { codes.push("7".to_string()); }
...
impl TerminalState {
    ...
    pub fn set_scrolling_region(&mut self, top: Option<usize>, bottom: Option<usize>) {
        ...
        let new_bottom = bottom.map(|b| b.saturating_sub(1)).unwrap_or(screen_height.saturating_sub(1));
            ...
            // Get buffer once for each iteration to avoid borrow checker issues
            {
                ...
                if top <= bottom && top < buffer.content.len() && bottom < buffer.content.len() {
                    ...
                    if bottom < buffer.content.len() {
                        let width = if buffer.content.len() > 0 { buffer.content[0].len() } else { screen_width };
    ...
    pub fn switch_to_alternate_screen(&mut self) {
        if !self.use_alternate_screen {
            ...
            // Clear alternate screen when switching to it
...
impl TerminalMessageProcessor {
    ...
    pub fn process_message(&mut self, message: TerminalMessage) {
        match message {
            ...
            TerminalMessage::SwitchScreenBuffer { to_alternate } => {
                if to_alternate {
                    self.ansi_processor.terminal_state.switch_to_alternate_screen();
                } else {
                    self.ansi_processor.terminal_state.switch_to_primary_screen();
                }
            }
            TerminalMessage::ClearScreen { mode } => {
                match mode {
                    ClearMode::Entire => self.ansi_processor.terminal_state.clear_screen(),
                    ClearMode::FromCursor => self.ansi_processor.terminal_state.get_active_buffer().clear_from_cursor_to_end(),
                    ClearMode::ToCursor => self.ansi_processor.terminal_state.get_active_buffer().clear_from_beginning_to_cursor(),
                    ClearMode::Scrollback => {
                        self.ansi_processor.terminal_state.get_active_buffer().scrollback.clear();
                    }
                }
            }
            ...
            TerminalMessage::MoveCursorRelative { dx, dy } => {
                self.ansi_processor.terminal_state.move_cursor_relative(dx, dy);
            }
            ...
            TerminalMessage::SetCursorVisibility { visible } => {
                self.ansi_processor.terminal_state.set_cursor_visibility(visible);
            }
            ...
            TerminalMessage::CursorPreviousLine { lines } => {
                self.ansi_processor.terminal_state.cursor_previous_line(lines);
            }
            ...
            TerminalMessage::SetScrollingRegion { top, bottom } => {
                ...
                self.ansi_processor.terminal_state.set_scrolling_region(Some(top + 1), Some(bottom + 1));
            }
            ...
            TerminalMessage::ScrollUpInRegion { lines } => {
                ...
                self.ansi_processor.terminal_state.scroll_up_in_region(lines);
            }
            TerminalMessage::ScrollDownInRegion { lines } => {
                // F0307: Scroll down within the current scrolling region
                self.ansi_processor.terminal_state.scroll_down_in_region(lines);
            }
            ...
            TerminalMessage::ContentUpdate { content, dirty_regions: _, replace_mode } => {
                if replace_mode {
    ...
    fn generate_device_status_report(&self, report_type: DeviceReportType) -> String {
        match report_type {
            DeviceReportType::Status => "\x1b[0n".to_string(), // Terminal OK
            ...
            _ => {
                log::debug!("Unhandled device status report: {:?}", report_type);
                String::new()
            }
        }
    }
...
impl AnsiProcessor {
    ...
    pub fn should_replace_content(&self) -> bool {
        self.terminal_state.full_screen_program_detected || self.alternate_screen_mode || self.terminal_state.use_alternate_screen
    }
    ...
    pub fn get_screen_content_for_stream(&self) -> String {
        if self.use_screen_buffer {
            ...
            self.terminal_state.get_active_buffer_ref().get_content_for_stream()
        } else {
    ...
    pub fn get_terminal_content_lines(&self, include_scrollback: bool) -> Vec<String> {
        if self.use_screen_buffer {
            self.terminal_state.get_active_buffer_ref().to_content_lines(include_scrollback)
        } else {
    ...
    pub fn get_state_changes(&self) -> Vec<TerminalMessage> {
        ...
        if self.terminal_state.use_alternate_screen {
            messages.push(TerminalMessage::SwitchScreenBuffer {
                to_alternate: true,
            });
        }
    ...
    fn calculate_dirty_regions(&self) -> Vec<DirtyRegion> {
        ...
        // TODO: Implement proper dirty region tracking
        vec![DirtyRegion {
            x: 0,
            y: 0,
    ...
    fn apply_sgr_param(&mut self, param: u16) {
        match param {
            ...
            61 => self.terminal_state.current_attributes.ideogram_double_underline = true,
            62 => self.terminal_state.current_attributes.ideogram_overline = true,
            63 => self.terminal_state.current_attributes.ideogram_double_overline = true,
            ...
            65 => {
                ...
                self.terminal_state.current_attributes.ideogram_double_underline = false;
                self.terminal_state.current_attributes.ideogram_overline = false;
                self.terminal_state.current_attributes.ideogram_double_overline = false;
                self.terminal_state.current_attributes.ideogram_stress = false;
            }
            
            // Bright/intense colors (90-97 foreground, 100-107 background)
            90..=97 => self.terminal_state.current_attributes.fg_color = Some((param - 90 + 8) as u8),
            100..=107 => self.terminal_state.current_attributes.bg_color = Some((param - 100 + 8) as u8),
                        ...
                        match mode {
                            5 => {
                                ...
                                if let Some(&palette_index) = param_iter.next() {
                                    self.terminal_state.current_attributes.fg_color = Some(palette_index as u8);
                                }
                            }
                            2 => {
                                ...
                                if let (Some(&r), Some(&g), Some(&b)) = (
                                    ...
                                    let palette_index = Self::rgb_to_palette(r as u8, g as u8, b as u8);
                                    self.terminal_state.current_attributes.fg_color = Some(palette_index);
                                }
                            }
                    ...
                    if let Some(&mode) = param_iter.next() {
                        match mode {
                            5 => {
                                ...
                                if let Some(&palette_index) = param_iter.next() {
                                    self.terminal_state.current_attributes.bg_color = Some(palette_index as u8);
                                }
                            }
                            2 => {
                                ...
                                if let (Some(&r), Some(&g), Some(&b)) = (
                                    ...
                                    // Convert RGB to nearest palette color for now
                                    let palette_index = Self::rgb_to_palette(r as u8, g as u8, b as u8);
                                    self.terminal_state.current_attributes.bg_color = Some(palette_index);
                                }
                            }
                            _ => {} // Unknown extended color mode
                        }
...
impl Perform for AnsiProcessor {
    ...
    fn execute(&mut self, byte: u8) {
        match byte {
            ...
            b'\t' => {
                let spaces = 8 - (self.terminal_state.get_cursor().x % 8);
                for _ in 0..spaces {
                    self.put_char_at_cursor(' ');
                    self.terminal_state.get_cursor_mut().x += 1;
                    if self.use_screen_buffer && self.terminal_state.get_cursor().x >= self.terminal_state.screen_width {
                        break;
                    }
                }
            }
        ...
        if parts.len() >= 2 {
            match parts[0] {
                "0" | "2" => {
                    ...
                    self.terminal_state.terminal_title = Some(title.clone());
                    log::debug!("Terminal title set to: {}", title);
                }
                "1" => {
                    // Set icon name
                    let icon = parts[1].to_string();
                    self.terminal_state.icon_name = Some(icon.clone());
                    log::debug!("Icon name set to: {}", icon);
                }
                "4" => {
                    // Set color palette (simplified - just log for now)
                    log::debug!("Color palette change: {}", parts[1]);
                }
                "10" => {
                    // Set foreground color
                    log::debug!("Foreground color change: {}", parts[1]);
                }
                "11" => {
                    // Set background color
                    log::debug!("Background color change: {}", parts[1]);
                }
                "52" => {
                    // Clipboard operations (simplified - just log for security)
                    log::debug!("Clipboard operation: {}", parts[1]);
                }
                _ => {
    ...
    fn csi_dispatch(&mut self, params: &Params, _intermediates: &[u8], _ignore: bool, c: char) {
        match c {
            ...
            'H' | 'f' => {
                ...
                self.terminal_state.set_cursor_position((col.saturating_sub(1)) as usize, (row.saturating_sub(1)) as usize);
            }
            'A' => {
                // Cursor up - CUU
                    ...
                    match n {
                        ...
                        1 => {
                            ...
                            for y in 0..=cursor_y.min(screen_height.saturating_sub(1)) {
                                let end_x = if y == cursor_y { cursor_x } else { screen_width };
            ...
            'K' => {
                ...
                if self.use_screen_buffer && self.terminal_state.get_cursor().y < self.terminal_state.screen_height {
                    // Extract values to avoid borrow checker issues
                    let cursor_x = self.terminal_state.get_cursor().x;
                    let cursor_y = self.terminal_state.get_cursor().y;
                    let screen_width = self.terminal_state.screen_width;
                ...
                if _intermediates.contains(&b'?') && params.len() > 0 {
                    ...
                    match mode {
                        ...
                        6 => {
                            ...
                            self.terminal_state.get_cursor_mut().x = 0;
                            self.terminal_state.get_cursor_mut().y = self.terminal_state.scroll_region.top;
                        }
                        7 => {
                        ...
                        1000 => {
                            // Mouse reporting - X10 mode
                            log::debug!("Mouse X10 mode enabled");
                        }
                        1002 => {
                            // Mouse reporting - Button Event mode
                            log::debug!("Mouse Button Event mode enabled");
                        }
                        1003 => {
                            // Mouse reporting - Any Event mode
                            log::debug!("Mouse Any Event mode enabled");
                        }
                        1006 => {
                            // Mouse reporting - SGR mode
                            log::debug!("Mouse SGR mode enabled");
                        }
                        1047 | 1049 => {
                            // Alternate screen buffer - definitive full-screen program
            ...
            'l' => {
                ...
                if _intermediates.contains(&b'?') && params.len() > 0 {
                    ...
                    match mode {
                        ...
                        1000 | 1002 | 1003 | 1006 => {
                            // Mouse reporting modes - disable
                            log::debug!("Mouse reporting mode {} disabled", mode);
                        }
                        1047 | 1049 => {
                ...
                if params.len() > 0 {
                    ...
                    match report_type {
                        ...
                        6 => {
                            ...
                            self.terminal_state.pending_response = Some(
                                format!("\x1b[{};{}R", cursor_y, cursor_x)
                            );
                        }
                        _ => {
            ...
            'c' => {
                ...
                if params.is_empty() || Self::get_param(params, 0) == 0 {
                    ...
                    self.terminal_state.pending_response = Some("\x1b[?6c".to_string());
    ...
    fn esc_dispatch(&mut self, intermediates: &[u8], _ignore: bool, byte: u8) {
        match byte {
            ...
            b'D' => {
                ...
                if self.terminal_state.get_cursor().y >= self.terminal_state.screen_height.saturating_sub(1) {
            ...
            b'M' => {
                ...
                if self.terminal_state.get_cursor().y == 0 {
                    ...
                    self.terminal_state.get_cursor_mut().y = self.terminal_state.get_cursor().y.saturating_sub(1);
                }
            }
            b'E' => {
            ...
            _ => {
                ...
                if intermediates.len() == 1 {
                    match intermediates[0] {
                        ...
                        _ => {
                            log::debug!("Unknown ESC intermediate sequence: {:?} {}", intermediates, byte as char);
                        }
                    }
                } else {
...
mod tests {
    ...
    fn test_terminal_message_architecture() {
        ...
        let message = TerminalMessage::ResizeTerminal { width: 100, height: 50 };
        ...
        assert_eq!(processor.ansi_processor.terminal_state.terminal_title, Some("Test Terminal".to_string()));
    }
    ...
    fn test_terminal_message_queue() {
        ...
        processor.queue_message(TerminalMessage::ResizeTerminal { width: 120, height: 40 });
    ...
    fn test_terminal_message_content_update() {
        ...
        let content_msg = TerminalMessage::ContentUpdate {
            ...
            dirty_regions: vec![DirtyRegion { x: 0, y: 0, width: 80, height: 1 }],
            replace_mode: true
        };
        ...
        assert_eq!(processor.ansi_processor.processed_text, "New content appended");
    }
    ...
    fn test_cursor_styles() {
        ...
        assert_eq!(terminal_state.get_cursor().style, CursorStyle::BlinkingBlock);
        
        terminal_state.set_cursor_style(CursorStyle::BlinkingUnderline);
        assert_eq!(terminal_state.get_cursor().style, CursorStyle::BlinkingUnderline);
    ...
    fn test_character_attribute_system() {
        ...
        assert_eq!(processor.terminal_state.current_attributes.bold, default_cell.bold);
        assert_eq!(processor.terminal_state.current_attributes.italic, default_cell.italic);
        assert_eq!(processor.terminal_state.current_attributes.underline, default_cell.underline);
        assert_eq!(processor.terminal_state.current_attributes.reverse, default_cell.reverse);
        assert_eq!(processor.terminal_state.current_attributes.strikethrough, default_cell.strikethrough);
    }
    ...
    fn test_standard_color_support() {
        ...
        assert_eq!(processor.terminal_state.current_attributes.fg_color, Some(0));
        
        processor.apply_sgr_param(31);  // Red
        assert_eq!(processor.terminal_state.current_attributes.fg_color, Some(1));
        
        processor.apply_sgr_param(37);  // White
        assert_eq!(processor.terminal_state.current_attributes.fg_color, Some(7));
        
        // Test bright colors (foreground)
        processor.apply_sgr_param(90);  // Bright black
        assert_eq!(processor.terminal_state.current_attributes.fg_color, Some(8));
        
        processor.apply_sgr_param(97);  // Bright white
        assert_eq!(processor.terminal_state.current_attributes.fg_color, Some(15));
        
        // Test standard 8 colors (background)
        processor.apply_sgr_param(40);  // Black background
        assert_eq!(processor.terminal_state.current_attributes.bg_color, Some(0));
        
        processor.apply_sgr_param(47);  // White background
        assert_eq!(processor.terminal_state.current_attributes.bg_color, Some(7));
        
        // Test bright colors (background)
        processor.apply_sgr_param(100); // Bright black background
        assert_eq!(processor.terminal_state.current_attributes.bg_color, Some(8));
        
        processor.apply_sgr_param(107); // Bright white background
        assert_eq!(processor.terminal_state.current_attributes.bg_color, Some(15));
    ...
    fn test_ideogram_attributes() {
        ...
        assert!(processor.terminal_state.current_attributes.ideogram_underline);
        
        processor.apply_sgr_param(61); // Ideogram double underline
        assert!(processor.terminal_state.current_attributes.ideogram_double_underline);
        
        processor.apply_sgr_param(62); // Ideogram overline
        assert!(processor.terminal_state.current_attributes.ideogram_overline);
        
        processor.apply_sgr_param(63); // Ideogram double overline
        assert!(processor.terminal_state.current_attributes.ideogram_double_overline);
    ...
    fn test_rgb_to_palette_conversion() {
        // Test primary colors
        assert_eq!(AnsiProcessor::rgb_to_palette(255, 0, 0), 16 + 36*5 + 6*0 + 0);   // Red
        assert_eq!(AnsiProcessor::rgb_to_palette(0, 255, 0), 16 + 36*0 + 6*5 + 0);   // Green
        assert_eq!(AnsiProcessor::rgb_to_palette(0, 0, 255), 16 + 36*0 + 6*0 + 5);   // Blue
        
        // Test white and black
        assert_eq!(AnsiProcessor::rgb_to_palette(255, 255, 255), 16 + 36*5 + 6*5 + 5); // White
        assert_eq!(AnsiProcessor::rgb_to_palette(0, 0, 0), 16 + 36*0 + 6*0 + 0);       // Black
        
        // Test mid-range color
        assert_eq!(AnsiProcessor::rgb_to_palette(128, 128, 128), 16 + 36*3 + 6*3 + 3); // Gray
    }
    ...
    fn test_scrolling_region_message_operations() {
        ...
        assert_eq!(processor.ansi_processor.terminal_state.scroll_region.bottom, 15); // 0-based (16-1)
        ...
        assert_eq!(processor.ansi_processor.terminal_state.scroll_region.bottom, 23); // Screen height - 1
