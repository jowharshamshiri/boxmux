...
mod debug_coordinate_issues {
    ...
    fn debug_coordinate_translation_mismatch() {
        ...
        let choices = muxbox
            .get_selected_stream_choices()
            .unwrap_or_else(|| &muxbox.choices.as_ref().unwrap());
        ...
        println!(
            "Testing click at screen ({}, {}) - this is the failing test click",
            test_click_x, test_click_y
        );
        ...
        for (i, zone) in old_translated_zones.iter().enumerate() {
            ...
            println!(
                "  OLD Zone {} contains screen ({}, {}): {}",
                i, test_click_x, test_click_y, contains
            );
            if contains {
        ...
        if let Some((inbox_x, inbox_y)) = dimensions.screen_to_inbox(test_click_x, test_click_y) {
            ...
            for (i, zone) in box_relative_zones.iter().enumerate() {
                ...
                println!(
                    "    Box-relative Zone {} contains inbox ({}, {}): {}",
                    i, inbox_x, inbox_y, contains
                );
                if contains {
            ...
            println!(
                "  CRITICAL TEST: Do OLD zones contain the screen coordinates used by draw_loop?"
            );
            let any_old_zone_contains = old_translated_zones
                .iter()
                .any(|z| z.bounds.contains_point(test_click_x, test_click_y));
            println!(
                "    Any OLD zone contains screen ({}, {}): {}",
                test_click_x, test_click_y, any_old_zone_contains
            );
            ...
            println!(
                "  NEW translation: screen ({}, {}) is outside content area",
                test_click_x, test_click_y
            );
        }
        ...
        println!(
            "Testing alternative click at screen ({}, {})",
            working_click_x, working_click_y
        );
        ...
        for (i, zone) in old_translated_zones.iter().enumerate() {
            ...
            println!(
                "  OLD Zone {} contains screen ({}, {}): {}",
                i, working_click_x, working_click_y, contains
            );
        }
