...
fn test_execution_mode_stream_behavioral_properties() {
    ...
    assert!(!ExecutionMode::Immediate.is_realtime(), "Immediate mode should not be real-time");
    assert!(!ExecutionMode::Immediate.is_background(), "Immediate mode should not be background");
    assert!(ExecutionMode::Immediate.creates_streams(), "Immediate mode MUST create streams");
    
    // Thread mode: not real-time, is background, creates streams
    assert!(!ExecutionMode::Thread.is_realtime(), "Thread mode should not be real-time");
    assert!(ExecutionMode::Thread.is_background(), "Thread mode should be background");
    assert!(ExecutionMode::Thread.creates_streams(), "Thread mode MUST create streams");
    
    // PTY mode: is real-time, is background, creates streams
    assert!(ExecutionMode::Pty.is_realtime(), "PTY mode should be real-time");
    assert!(ExecutionMode::Pty.is_background(), "PTY mode should be background");
    assert!(ExecutionMode::Pty.creates_streams(), "PTY mode MUST create streams");
}
...
fn test_execution_mode_choice_integration() {
    ...
    assert!(choice.execution_mode.creates_streams(), 
...
fn test_execution_mode_serialization() {
    ...
    for mode in modes {
        ...
        let deserialized: ExecutionMode = serde_json::from_str(&serialized)
            .expect("Failed to deserialize ExecutionMode");
        
        assert_eq!(mode, deserialized, 
            "ExecutionMode serialization roundtrip failed for {:?}", mode);
        
        // Ensure stream creation property is preserved
        assert_eq!(mode.creates_streams(), deserialized.creates_streams(),
            "Stream creation property changed during serialization for {:?}", mode);
    }
}
...
fn test_execution_mode_description() {
    ...
    assert!(ExecutionMode::Immediate.description().to_lowercase().contains("synchronous"),
        "Immediate mode description should mention synchronous execution");
    assert!(ExecutionMode::Thread.description().to_lowercase().contains("background"),
        "Thread mode description should mention background execution");
    assert!(ExecutionMode::Pty.description().to_lowercase().contains("real-time"),
...
fn test_execution_mode_default() {
    ...
    assert!(default_mode.creates_streams(),
