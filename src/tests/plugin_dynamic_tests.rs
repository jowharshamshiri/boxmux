...
mod plugin_dynamic_tests {
    ...
    fn test_plugin_manifest_loading_mock() {
        ...
        let manifest = registry
            .load_manifest(temp_dir.path().join("nonexistent.toml"))
            .unwrap();
...
"#;
        ...
        assert_eq!(
            manifest.component_types,
            vec!["advanced_chart", "data_widget"]
        );
...
"#;
        ...
        match result {
            ...
            Err(PluginError::InitializationFailed(msg)) => {
                ...
                panic!(
                    "Expected PermissionDenied or parsing error, got InitializationFailed: {}",
                    msg
                );
            }
            other => {
                panic!(
                    "Expected PermissionDenied or parsing error, got: {:?}",
                    other
                );
            }
        }
...
"#;
        ...
        assert!(result
            .unwrap()
            .contains("Custom component: missing_component"));
    }
