...
mod layout_validation_tests {
    use crate::model::app::{App, load_app_from_yaml};
    ...
    fn test_all_layout_files_are_valid() {
        ...
        assert!(!yaml_files.is_empty(), "Should find at least one YAML file in layouts directory");
        ...
        for yaml_file in yaml_files {
            total_files += 1;
            let file_name = yaml_file.file_name()
                .and_then(|n| n.to_str())
                .unwrap_or("unknown");
            ...
            assert!(app_context.app.get_root_layout().is_some(), 
                   "File {} should have a root layout", file_name);
            
            // Verify at least one layout exists
            assert!(!app_context.app.layouts.is_empty(), 
                   "File {} should have at least one layout", file_name);
            
            successful_loads += 1;
        }
        
        println!("Layout validation summary: {}/{} files successfully loaded and validated", 
                successful_loads, total_files);
        
        // Verify we successfully processed all files
        assert_eq!(successful_loads, total_files, 
                  "All layout files should load successfully");
        
        // Ensure we tested a reasonable number of files
        assert!(total_files >= 10, 
               "Expected at least 10 layout files, found {}", total_files);
    }
    ...
    fn test_specific_layout_files_functionality() {
        let test_cases = vec![
            ...
            ("system_monitor_pro.yaml", "Should contain system monitoring panels"),
            ("developer_workspace.yaml", "Should contain development tools"),
        ];
        ...
        for (filename, description) in test_cases {
            ...
            println!("Testing specific functionality of {}: {}", filename, description);
            ...
            let root_layout = app.layouts.iter().find(|l| l.root == Some(true))
                .unwrap_or_else(|| panic!("{} should have a root layout", filename));
            
            assert!(root_layout.children.is_some(), 
                   "{} root layout should have children", filename);
            
            let children = root_layout.children.as_ref().unwrap();
            assert!(!children.is_empty(), 
                   "{} should have at least one panel", filename);
            
            // File-specific validations
            match filename {
                "chart_demo.yaml" => {
                    let has_charts = children.iter().any(|panel| {
                        panel.chart_type.is_some() || 
                        panel.chart_data.is_some()
                    });
                    assert!(has_charts, "chart_demo.yaml should contain chart panels");
                },
                "table_demo.yaml" => {
                    let has_tables = children.iter().any(|panel| {
                        panel.table_data.is_some() || 
                        panel.table_config.is_some()
                    });
                    assert!(has_tables, "table_demo.yaml should contain table panels");
                },
                "plugin_demo.yaml" => {
                    let has_plugins = children.iter().any(|panel| {
                        panel.plugin_component.is_some()
                    });
            ...
            println!("  ✅ Specific functionality validation passed for {}", filename);
        }
    }
            ...
            for layout in &app.layouts {
                if let Some(children) = &layout.children {
                    ...
                    for panel in children {
                        assert!(panel_ids.insert(&panel.id), 
                               "{} layout '{}' has duplicate panel ID: {}", 
                               file_name, layout.id, panel.id);
                    }
                }
            }
    ...
    fn test_layout_files_performance() {
        ...
        for yaml_file in &yaml_files {
            ...
            let file_path_str = yaml_file.to_str().expect("Invalid file path");
            let _app = load_app_from_yaml(file_path_str)
                .expect("App should load successfully");
            
            let load_time = file_start.elapsed();
            
            // Each file should load within reasonable time (1.5 seconds to account for system variation)
            assert!(load_time.as_millis() < 1500, 
                   "File {:?} took too long to load: {}ms", 
                   yaml_file.file_name(), load_time.as_millis());
        }
        
        let total_time = start_time.elapsed();
        
        println!("Performance test: loaded {} layout files in {}ms", 
                yaml_files.len(), total_time.as_millis());
        
        // All files combined should load within reasonable time (5 seconds)
        assert!(total_time.as_secs() < 5, 
               "Total loading time too long: {}ms", total_time.as_millis());
    }
}