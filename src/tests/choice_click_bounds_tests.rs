...
mod choice_click_bounds_tests {
    ...
    fn test_click_on_choice_text_triggers_choice() {
        ...
        assert_eq!(result, Some(0), "Should trigger first choice when clicking on first character");
        ...
        assert_eq!(result, Some(0), "Should trigger first choice when clicking on last character");
        
        // Click on "R" of "Run Tests" (first character of second choice)
        let result = calculate_clicked_choice_index(&muxbox, 12, 12, &choices);
        assert_eq!(result, Some(1), "Should trigger second choice when clicking on first character");
    }
    ...
    fn test_click_after_choice_text_does_not_trigger() {
        ...
        assert_eq!(result, None, "Should not trigger choice when clicking after text");
        
        // Click far to the right on same line
        let result = calculate_clicked_choice_index(&muxbox, 19, 11, &choices);
        assert_eq!(result, None, "Should not trigger choice when clicking far after text");
    }
    ...
    fn test_click_before_choice_text_does_not_trigger() {
        ...
        assert_eq!(result, None, "Should not trigger choice when clicking before text");
        
        // Click at muxbox left border
        let result = calculate_clicked_choice_index(&muxbox, 10, 11, &choices);
        assert_eq!(result, None, "Should not trigger choice when clicking on muxbox border");
    }
    ...
    fn test_click_on_waiting_choice_text() {
        ...
        assert_eq!(result, Some(0), "Should trigger waiting choice when clicking within expanded text");
        
        // Click after the "..." should not trigger
        let result = calculate_clicked_choice_index(&muxbox, 21, 11, &choices);
        assert_eq!(result, None, "Should not trigger waiting choice when clicking after expanded text");
    }
    ...
    fn test_click_on_different_choice_lines() {
        ...
        assert_eq!(result, None, "Should not trigger when clicking after third choice text");
    }
    ...
    fn test_coordinate_boundary_conditions() {
        ...
        let result = calculate_clicked_choice_index(&muxbox, 12, 11, &choices);
        assert_eq!(result, Some(0), "Should trigger choice when clicking exactly on single character");
        
        // Click one position after
        let result = calculate_clicked_choice_index(&muxbox, 13, 11, &choices);
        assert_eq!(result, None, "Should not trigger choice when clicking one position after single character");
    }
}