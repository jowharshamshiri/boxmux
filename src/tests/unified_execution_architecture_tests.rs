...
pub mod unified_execution_architecture_tests {
    ...
    fn test_unified_execution_all_modes_use_thread_manager() {
        ...
        for mode in modes {
            ...
            println!("✓ Mode {:?} uses consistent stream ID format: {}", mode, expected_stream_id);
        }
    }
    ...
    fn test_pty_mode_supports_multiple_commands() {
        ...
        println!("✓ PTY mode supports {} commands", choice.script.as_ref().unwrap().len());
    ...
    fn test_multiple_clicks_create_multiple_streams() {
        ...
        assert_eq!(stream_ids.len(), unique_ids.len(), "All stream IDs should be unique");
    ...
    fn test_legacy_boolean_fields_ignored() {
        ...
        println!("✓ Stream suffix correctly uses ExecutionMode: {}", stream_suffix);
    }
    ...
    fn test_consistent_stream_labeling() {
        ...
        for (mode, expected_suffix) in modes {
            ...
            println!("✓ Mode {:?}: stream_id={}, label={}", mode, stream_id, expected_label);
        }
    }
}