use crate::components::{Border, BorderStyle, BorderCharSet};
use crate::Bounds;
...
fn test_pty_color_calculation() {
    ...
    let pty_border = Border::new(BorderStyle::Single, Some(7), None)
        .with_pty_state(true, false, false);
    
    let color = pty_border.calculate_border_color();
    assert_eq!(color, "14");
    
    // Test error state - should return "9"
    let error_border = Border::new(BorderStyle::Single, Some(7), None)
        .with_pty_state(false, true, false);
...
fn test_custom_charset() {
    ...
    let border = Border::new(BorderStyle::Custom(custom_charset.clone()), Some(7), Some(0));
