...
fn test_muxbox_bounds_calculation_within_terminal() {
    ...
    assert!(calculated_bounds.x1 >= 0, 
        "MuxBox x1 {} is negative", calculated_bounds.x1);
    assert!(calculated_bounds.y1 >= 0,
        "MuxBox y1 {} is negative", calculated_bounds.y1);
    assert!(calculated_bounds.x2 <= terminal_bounds.x2,
        "MuxBox x2 {} exceeds terminal width {}", calculated_bounds.x2, terminal_bounds.x2);
    assert!(calculated_bounds.y2 <= terminal_bounds.y2,
        "MuxBox y2 {} exceeds terminal height {}", calculated_bounds.y2, terminal_bounds.y2);
        
    // Verify muxbox has positive dimensions
    assert!(calculated_bounds.x2 > calculated_bounds.x1,
        "MuxBox width is not positive: x1={}, x2={}", calculated_bounds.x1, calculated_bounds.x2);
    assert!(calculated_bounds.y2 > calculated_bounds.y1,
        "MuxBox height is not positive: y1={}, y2={}", calculated_bounds.y1, calculated_bounds.y2);
...
fn test_muxbox_with_title_content_bounds() {
    ...
    assert!(calculated_bounds.x1 >= 0, "MuxBox x1 should be non-negative");
    assert!(calculated_bounds.y1 >= 0, "MuxBox y1 should be non-negative");
    assert!(calculated_bounds.x2 <= terminal_bounds.x2, 
    ...
    assert!(muxbox_width >= 10, "MuxBox width {} should be at least 10 for title", muxbox_width);
    assert!(muxbox_height >= 3, "MuxBox height {} should be at least 3 for title + content", muxbox_height);
...
fn test_minimal_muxbox_bounds_handling() {
    ...
    assert!(muxbox_width >= 0, "Tiny muxbox width should be non-negative");
    assert!(muxbox_height >= 0, "Tiny muxbox height should be non-negative");
...
fn test_muxbox_with_scroll_bounds_validation() {
    ...
    assert!(muxbox_width > 0, "Scrollable muxbox width should be positive");
    assert!(muxbox_height > 0, "Scrollable muxbox height should be positive");
...
fn test_muxbox_with_chart_bounds_validation() {
    ...
    assert!(muxbox_width >= 20, "Chart muxbox width {} should be at least 20", muxbox_width);
    assert!(muxbox_height >= 10, "Chart muxbox height {} should be at least 10", muxbox_height);
...
fn test_muxbox_with_table_bounds_validation() {
    ...
    muxbox.content = Some(r#"Name,Age,Email,Department,Position,Location,Salary
...
Alice Brown,32,alice.brown@example.com,Engineering,Tech Lead,Seattle,90000"#.to_string());
    ...
    assert!(muxbox_width >= 30, "Table muxbox width {} should be at least 30", muxbox_width);
    assert!(muxbox_height >= 5, "Table muxbox height {} should be at least 5", muxbox_height);
...
fn test_muxbox_bounds_with_different_anchors() {
    ...
    for anchor in anchors {
        ...
        // Verify bounds are within terminal
        assert!(calculated_bounds.x1 >= 0, 
            "MuxBox with anchor {:?} has x1={} < 0", anchor, calculated_bounds.x1);
        assert!(calculated_bounds.y1 >= 0,
            "MuxBox with anchor {:?} has y1={} < 0", anchor, calculated_bounds.y1);
        assert!(calculated_bounds.x2 <= terminal_bounds.x2,
            "MuxBox with anchor {:?} has x2={} exceeding terminal width {}", anchor, calculated_bounds.x2, terminal_bounds.x2);
        assert!(calculated_bounds.y2 <= terminal_bounds.y2,
            "MuxBox with anchor {:?} has y2={} exceeding terminal height {}", anchor, calculated_bounds.y2, terminal_bounds.y2);
        
        // Verify muxbox has positive dimensions
        assert!(calculated_bounds.x2 > calculated_bounds.x1,
            "MuxBox with anchor {:?} has invalid width: x1={}, x2={}", anchor, calculated_bounds.x1, calculated_bounds.x2);
        assert!(calculated_bounds.y2 > calculated_bounds.y1,
            "MuxBox with anchor {:?} has invalid height: y1={}, y2={}", anchor, calculated_bounds.y1, calculated_bounds.y2);
    }
