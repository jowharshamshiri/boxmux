...
mod choice_execution_debug_tests {
    ...
    fn debug_multi_stream_choice_execution() {
        ...
        tester
            .load_config("layouts/multi_stream_tabs_demo.yaml")
        ...
        println!(
            "Initial frame captured - multi_stream_box should be visible with {} tabs",
            initial_tabs
        );
        ...
        println!(
            "Clicking control_panel at ({}, {})",
            control_panel_click.column, control_panel_click.row
        );
        tester
            .send_mouse_event(
                MouseEventKind::Down(MouseButton::Left),
                control_panel_click.column,
                control_panel_click.row,
            )
            .expect("Failed to send mouse click to control_panel");

        let after_focus_frame = tester
            .wait_for_frame()
        ...
        println!(
            "Clicking 'Add External Stream' choice at ({}, {})",
            external_choice_click.column, external_choice_click.row
        );
        tester
            .send_mouse_event(
                MouseEventKind::Down(MouseButton::Left),
                external_choice_click.column,
                external_choice_click.row,
            )
            .expect("Failed to click Add External Stream choice");

        // Wait to see the result
        std::thread::sleep(Duration::from_millis(2000));
        let after_external_frame = tester
            .wait_for_frame()
        ...
        println!(
            "Clicking multi_stream_box at ({}, {})",
            multi_stream_click.column, multi_stream_click.row
        );
        tester
            .send_mouse_event(
                MouseEventKind::Down(MouseButton::Left),
                multi_stream_click.column,
                multi_stream_click.row,
            )
            .expect("Failed to click multi_stream_box");

        let _after_multi_focus_frame = tester
            .wait_for_frame()
        ...
        println!(
            "Clicking 'Deploy App' choice at ({}, {})",
            deploy_choice_click.column, deploy_choice_click.row
        );
        tester
            .send_mouse_event(
                MouseEventKind::Down(MouseButton::Left),
                deploy_choice_click.column,
                deploy_choice_click.row,
            )
            .expect("Failed to click Deploy App choice");

        // Wait to see the result
        std::thread::sleep(Duration::from_millis(3000));
        let after_deploy_frame = tester
            .wait_for_frame()
        ...
        println!(
            "Clicking 'Monitor Logs' choice at ({}, {})",
            monitor_choice_click.column, monitor_choice_click.row
        );
        tester
            .send_mouse_event(
                MouseEventKind::Down(MouseButton::Left),
                monitor_choice_click.column,
                monitor_choice_click.row,
            )
            .expect("Failed to click Monitor Logs choice");

        // Wait to see the result
        std::thread::sleep(Duration::from_millis(6000)); // Longer wait for the loop script
        let after_monitor_frame = tester
            .wait_for_frame()
        ...
        let has_monitor_output =
            frame_contains_text(&after_monitor_frame, "Monitoring application logs");
        ...
        println!(
            "Clicking 'Start PTY Process' choice at ({}, {})",
            pty_choice_click.column, pty_choice_click.row
        );
        tester
            .send_mouse_event(
                MouseEventKind::Down(MouseButton::Left),
                pty_choice_click.column,
                pty_choice_click.row,
            )
            .expect("Failed to click Start PTY Process choice");

        // Wait to see the result
        std::thread::sleep(Duration::from_millis(3000));
        let after_pty_frame = tester
            .wait_for_frame()
        ...
        let has_pty_output = frame_contains_text(&after_pty_frame, "zenith")
            || frame_contains_text(&after_pty_frame, "PTY");
        ...
        assert!(
            true,
            "Debug test completed - check console output for analysis"
        );
    }

    /// Helper function to count tabs in a specific box by looking for tab indicators
    fn count_tabs_in_frame(
        frame: &crate::tests::visual_testing::terminal_capture::TerminalFrame,
        box_title: &str,
    ) -> usize {
