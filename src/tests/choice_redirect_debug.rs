...
mod choice_redirect_debug {
    ...
    fn analyze_choice_configurations() {
        ...
        let multi_stream_box = app_context
            .app
            .get_muxbox_by_id("multi_stream_box")
            .expect("multi_stream_box not found");
        let control_panel = app_context
            .app
            .get_muxbox_by_id("control_panel")
        ...
        println!(
            "  Choice count: {}",
            multi_stream_box.choices.as_ref().map_or(0, |c| c.len())
        );
        ...
        println!(
            "  Choice count: {}",
            control_panel.choices.as_ref().map_or(0, |c| c.len())
        );
            ...
            for choice in choices {
                if let Some(ref redirect) = choice.redirect_output {
                    if redirect != expected_target {
                        ...
                        println!(
                            "❌ control_panel choice '{}' redirects to '{}', expected '{}'",
                            choice.id, redirect, expected_target
                        );
                    }
                } else {
                    println!(
                        "❌ control_panel choice '{}' has no redirect_output",
                        choice.id
                    );
                }
            }
            ...
            for choice in choices {
                if let Some(ref redirect) = choice.redirect_output {
                    if redirect != expected_target {
                        all_redirect_to_same = false;
                        println!(
                            "❌ multi_stream_box choice '{}' redirects to '{}', expected '{}'",
                            choice.id, redirect, expected_target
                        );
                    }
                } else {
                    println!(
                        "❌ multi_stream_box choice '{}' has no redirect_output",
                        choice.id
                    );
                }
            }
        ...
        if all_redirect_to_same {
            println!(
                "✅ All choices redirect to the same target: '{}'",
                expected_target
            );
        }
        ...
        assert!(
            true,
            "Analysis complete - check console output for findings"
        );
    }
    ...
    fn analyze_script_complexity() {
        ...
        let pty_script = vec!["zenith".to_string()];

        println!(
            "  Working (external_stream): {} commands, simple echo",
            working_script.len()
        );
        println!("  Deploy: {} commands, includes sleep", deploy_script.len());
        println!(
            "  Monitor: {} commands, includes loop and sleep",
            monitor_script.len()
        );
        println!(
            "  PTY: {} commands, single command 'zenith'",
            pty_script.len()
        );
