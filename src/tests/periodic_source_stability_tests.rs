...
fn test_periodic_sources_get_stable_stream_ids() {
    ...
    assert_eq!(
        ...
        "Periodic refresh sources should have stable stream IDs"
    );
    ...
    assert_eq!(
        sources_for_box.len(),
        1,
        "Should have exactly one periodic source per box"
    );
...
fn test_different_boxes_get_different_periodic_stream_ids() {
    ...
    assert_ne!(
        stream_id_1, stream_id_2,
        "Different boxes should have different stream IDs"
    );

    // Verify we have two sources registered
    assert_eq!(
        app.execution_sources.len(),
        2,
        "Should have two registered sources"
    );
}
...
fn test_boxes_without_scripts_get_no_periodic_sources() {
    ...
    assert_eq!(
        app.execution_sources.len(),
        0,
        "Boxes without scripts should not get periodic sources"
    );
    ...
    assert_eq!(
        sources_for_box.len(),
        0,
        "Box without script should have no sources"
    );
}
