...
mod tests {
    ...
    fn create_test_muxbox_with_z_index(id: &str, x1: i32, y1: i32, x2: i32, y2: i32, z_index: Option<i32>) -> MuxBox {
    ...
    fn test_effective_z_index_explicit() {
        ...
        assert_eq!(muxbox.effective_z_index(), 5, "Explicit z_index should be returned");
    }
    ...
    fn test_z_index_sorting_for_rendering() {
        ...
        assert_eq!(sorted_boxes[0].id, "box3", "Box with z_index 0 should be first");
        assert_eq!(sorted_boxes[1].id, "box2", "Box with z_index 5 should be second");
        assert_eq!(sorted_boxes[2].id, "box1", "Box with z_index 10 should be last (on top)");
    }
    ...
    fn test_z_index_click_detection_priority() {
        ...
        assert!(clicked_box.is_some(), "Should find a box at click coordinates");
        assert_eq!(clicked_box.unwrap().id, "high", "Should click the box with highest z_index (10)");
    }
    ...
    fn test_z_index_with_nested_boxes() {
        ...
        assert_eq!(clicked_box.unwrap().id, "child2", "Should click child with highest z_index (15)");
    }
    ...
    fn test_z_index_serialization() {
        ...
        let json_with_z = serde_json::to_string(&muxbox_with_z).expect("Should serialize with z_index");
        let json_without_z = serde_json::to_string(&muxbox_without_z).expect("Should serialize without z_index");

        assert!(json_with_z.contains("\"z_index\":42"), "JSON should include z_index field");
        // Note: serde includes None Option fields by default, so we check for null value instead
        assert!(json_without_z.contains("\"z_index\":null") || !json_without_z.contains("\"z_index\""), "JSON should have z_index as null or omitted when None");

        // Test deserialization from JSON
        let deserialized_with_z: MuxBox = serde_json::from_str(&json_with_z).expect("Should deserialize with z_index");
        let deserialized_without_z: MuxBox = serde_json::from_str(&json_without_z).expect("Should deserialize without z_index");

        assert_eq!(deserialized_with_z.z_index, Some(42), "Deserialized z_index should match");
        assert_eq!(deserialized_without_z.z_index, None, "Deserialized z_index should be None");
    }
    ...
    fn test_z_index_hash_equality() {
        ...
        assert_ne!(box1, box3, "Boxes with different z_index should not be equal");
        ...
        assert_eq!(hasher1.finish(), hasher2.finish(), "Boxes with same z_index should have same hash");
        assert_ne!(hasher1.finish(), hasher3.finish(), "Boxes with different z_index should have different hash");
    }
    ...
    fn test_negative_z_index() {
        ...
        assert_eq!(sorted_boxes[0].id, "negative", "Negative z_index should render first (behind)");
        assert_eq!(sorted_boxes[1].id, "default", "Default z_index (0) should render second");
        assert_eq!(sorted_boxes[2].id, "positive", "Positive z_index should render last (on top)");
    }
}