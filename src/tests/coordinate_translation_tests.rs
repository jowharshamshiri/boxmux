...
mod coordinate_translation_tests {
    ...
    fn test_basic_coordinate_translation() {
        ...
        if let Some((inbox_x, inbox_y)) = dimensions.screen_to_inbox(screen_x, screen_y) {
            ...
            assert!(
                inbox_x < content_width,
                "Inbox X should be within content width"
            );
            assert!(
                inbox_y < content_height,
                "Inbox Y should be within content height"
            );
        }
    ...
    fn test_boundary_conditions() {
        ...
        assert!(
            dimensions.screen_to_inbox(50, 50).is_none(),
            "Far outside bounds should be None"
        );

        // Test border areas - should return None
        assert!(
            dimensions.screen_to_inbox(10, 5).is_none(),
    ...
    fn test_coordinate_system_consistency() {
        ...
        let test_points = vec![(10, 8), (15, 10), (20, 12)];
        ...
        for (screen_x, screen_y) in test_points {
            if let Some((inbox_x, inbox_y)) = dimensions.screen_to_inbox(screen_x, screen_y) {
                ...
                assert!(
                    x_diff <= 3,
                    "Round-trip X consistency failed for ({},{}) -> ({},{}) -> ({},{}) diff:{}",
                    screen_x,
                    screen_y,
                    inbox_x,
                    inbox_y,
                ...
                assert!(
                    y_diff <= 3,
                    "Round-trip Y consistency failed for ({},{}) -> ({},{}) -> ({},{}) diff:{}",
                    screen_x,
                    screen_y,
                    inbox_x,
                    inbox_y,
