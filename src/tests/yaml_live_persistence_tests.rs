...
use crate::model::app::{save_active_layout_to_yaml, save_complete_state_to_yaml, save_muxbox_bounds_to_yaml, save_muxbox_content_to_yaml, save_muxbox_scroll_to_yaml};
...
"#;
    ...
    let yaml_value: serde_yaml::Value = serde_yaml::from_str(&updated_content)
        .expect("Failed to parse updated YAML");
                        ...
                        for child in children {
                            if let serde_yaml::Value::Mapping(box_map) = child {
                                if let Some(serde_yaml::Value::String(id)) = box_map.get("id") {
                                    if id == "test_box" {
                                        found_box = true;
                                        if let Some(serde_yaml::Value::Mapping(pos)) = box_map.get("position") {
                                            assert_eq!(pos.get("x1"), Some(&serde_yaml::Value::String("10%".to_string())));
                                            assert_eq!(pos.get("y1"), Some(&serde_yaml::Value::String("10%".to_string())));
                                            assert_eq!(pos.get("x2"), Some(&serde_yaml::Value::String("60%".to_string())));
                                            assert_eq!(pos.get("y2"), Some(&serde_yaml::Value::String("60%".to_string())));
                                        }
                                    }
                                }
                            }
...
"#;
    ...
    let yaml_value: serde_yaml::Value = serde_yaml::from_str(&updated_content_str)
        .expect("Failed to parse updated YAML");
                    ...
                    if let Some(serde_yaml::Value::Sequence(children)) = layout_map.get("children") {
                        for child in children {
                            if let serde_yaml::Value::Mapping(box_map) = child {
                                if let Some(serde_yaml::Value::String(id)) = box_map.get("id") {
                                    if id == "content_box" {
                                        if let Some(serde_yaml::Value::String(content)) = box_map.get("content") {
                    ...
                    if let Some(serde_yaml::Value::Sequence(children)) = layout_map.get("children") {
                        for child in children {
                            if let serde_yaml::Value::Mapping(box_map) = child {
                                if let Some(serde_yaml::Value::String(id)) = box_map.get("id") {
                                    if id == "scroll_box" {
                                        if let Some(serde_yaml::Value::Number(scroll_x)) = box_map.get("scroll_x") {
                                            assert_eq!(scroll_x.as_u64(), Some(10));
                                        }
                                        if let Some(serde_yaml::Value::Number(scroll_y)) = box_map.get("scroll_y") {
...
fn test_complete_state_yaml_persistence() {
    ...
    assert!(loaded_app_result.is_ok(), "Should be able to load saved state");
...
"#;
    ...
    assert!(!PathBuf::from(&temp_atomic_path).exists(), "Temporary file should be cleaned up after atomic write");
