...
mod tests {
    ...
    fn test_socket_json_message_processing() {
        ...
        assert!(json_result.is_ok(), "Socket function should serialize to JSON");
        
        let json_string = json_result.unwrap();
        assert!(!json_string.is_empty(), "JSON string should not be empty");
        assert!(json_string.contains("test_panel"), "JSON should contain panel ID");
        assert!(json_string.contains("Socket test content"), "JSON should contain content");
        
        // Test deserialization
        let deserialize_result: Result<SocketFunction, _> = serde_json::from_str(&json_string);
        assert!(deserialize_result.is_ok(), "JSON should deserialize back to SocketFunction");
        
        let deserialized = deserialize_result.unwrap();
        assert_eq!(socket_function, deserialized, "Roundtrip should preserve data");
        
        // Test processing the socket function
        let app_context = TestDataFactory::create_test_app_context();
        let processing_result = crate::model::common::run_socket_function(deserialized, &app_context);
        assert!(processing_result.is_ok(), "Socket function should process successfully");
    ...
    fn test_socket_concurrent_connections() {
        ...
        assert_eq!(successful_operations, 3, "All concurrent socket function operations should succeed");
    }
    ...
    fn test_socket_error_handling() {
        ...
        for test_message in test_cases {
            ...
            if let Ok(socket_function) = json_parse_result {
                let app_context = TestDataFactory::create_test_app_context();
                let processing_result = crate::model::common::run_socket_function(socket_function, &app_context);
                
                // Processing should either succeed or fail gracefully
                match processing_result {
                    Ok((_, messages)) => {
                        // If successful, should produce at least one message
                        assert!(!messages.is_empty(), "Successful processing should produce messages");
                    }
                    Err(_) => {
        ...
        for whitespace in whitespace_cases {
            ...
            assert!(trimmed.is_empty() || !trimmed.is_empty(), "Whitespace handling should not panic");
        }
    }
    ...
    fn test_socket_cli_integration() {
        ...
        for (i, socket_function) in socket_functions.iter().enumerate() {
            let json_result = serde_json::to_string(socket_function);
            assert!(json_result.is_ok(), "Socket function {} should serialize to JSON", i);
            ...
            let deserialized = deserialize_result.unwrap();
            assert_eq!(*socket_function, deserialized, "Roundtrip serialization should preserve data");
        }
    }
    ...
    fn test_external_message_integration() {
        ...
        for message in test_messages {
            ...
            if !trimmed.is_empty() {
                ...
                assert!(!trimmed.is_empty(), "Non-empty messages should be processed");
            }
        }
    }
    ...
    fn test_socket_performance() {
        ...
        println!("Socket function performance: {} successful operations out of {} attempts in {:?}", 
        ...
        assert!(duration.as_millis() < 1000, "Socket function performance regression: {:?}", duration);
    }
}