...
fn test_unified_execution_immediateediate_mode() {
    ...
    assert!(
        muxbox.streams.len() >= initial_stream_count,
...
fn test_unified_execution_redirect_output() {
    ...
    source_muxbox.choices = Some(vec![Choice {
...
fn test_action_only_boxes_no_content_streams() {
    ...
    action_box.choices = Some(vec![Choice {
    ...
    let has_content_stream = action_box
        .streams
        .values()
        .any(|s| matches!(s.stream_type, crate::model::common::StreamType::Content));
    assert!(
        !has_content_stream,
        "Action-only boxes should not have Content streams"
    );

    // Should only have Choices stream
    let has_choices_stream = action_box
        .streams
        .values()
        .any(|s| matches!(s.stream_type, crate::model::common::StreamType::Choices));
    assert!(
        has_choices_stream,
        "Action-only boxes should have Choices stream"
    );

    // Should have exactly 1 stream (choices only)
    assert_eq!(
        action_box.streams.len(),
        1,
        "Action-only boxes should have exactly 1 stream"
    );
...
fn test_mixed_content_and_choices_boxes() {
    ...
    mixed_box.choices = Some(vec![Choice {
    ...
    let has_content_stream = mixed_box
        .streams
        .values()
        .any(|s| matches!(s.stream_type, crate::model::common::StreamType::Content));
    assert!(
        has_content_stream,
        "Mixed boxes should have Content stream for meaningful content"
    );

    let has_choices_stream = mixed_box
        .streams
        .values()
        .any(|s| matches!(s.stream_type, crate::model::common::StreamType::Choices));
    ...
    assert_eq!(
        mixed_box.streams.len(),
        2,
        "Mixed boxes should have exactly 2 streams"
    );

    // Content stream should be active by default
    let content_stream = mixed_box
        .streams
        .values()
        .find(|s| matches!(s.stream_type, crate::model::common::StreamType::Content));
    assert!(content_stream.is_some(), "Should have content stream");
    assert!(
        content_stream.unwrap().active,
        "Content stream should be active in mixed boxes"
    );
}
...
fn test_execution_stream_id_generation() {
    ...
    for (mode, expected_suffix) in test_cases {
        ...
        assert_eq!(
            actual_stream_id, expected_stream_id,
            "Stream ID generation should be consistent for {:?} mode",
            mode
        );
    }
}
...
fn test_no_unwanted_timestamp_content() {
    ...
    assert!(
        !expected_stream_id.contains(":"),
        "Stream ID should not contain timestamp characters"
    );
}
...
fn test_execution_mode_consistency_across_trigger_methods() {
    ...
    let expected_stream_id = format!(
        "{}_{}",
        immediate_choice.id,
        immediate_choice.execution_mode.as_stream_suffix()
    );
