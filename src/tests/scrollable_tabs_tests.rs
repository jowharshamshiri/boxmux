...
mod tests {
    ...
    use crate::draw_utils::{calculate_tab_click_index, calculate_tab_navigation_click, TabNavigationAction};
    ...
    fn test_tabs_need_scrolling_detection() {
        ...
        assert!(muxbox.tabs_need_scrolling(50), "Should need scrolling with 10 tabs in 50 characters");
        
        // With large width, should not need scrolling  
        assert!(!muxbox.tabs_need_scrolling(200), "Should not need scrolling with 10 tabs in 200 characters");
        
        // Empty muxbox should not need scrolling
        let empty_muxbox = TestDataFactory::create_test_muxbox("empty");
        assert!(!empty_muxbox.tabs_need_scrolling(50), "Empty muxbox should not need scrolling");
    }
    ...
    fn test_tab_scroll_boundaries() {
        ...
        assert!(muxbox.tab_scroll_offset <= 2, "Should not scroll past last tab");
    }
    ...
    fn test_max_tab_scroll_offset() {
        ...
        assert!(max_offset > 0, "Should have positive max offset with many tabs");
        ...
        assert_eq!(max_offset_large, 0, "Should have max offset 0 when all tabs fit");
    }

    #[test]
    fn test_tab_click_with_scrolling() {
        let tab_labels = vec![
            "Tab1".to_string(), "Tab2".to_string(), "Tab3".to_string(),
        ...
        if let Some(tab_index) = clicked_tab {
            assert!(tab_index >= 2, "Should click tabs accounting for scroll offset");
        }
    }
    ...
    fn test_navigation_arrow_detection() {
        let tab_labels = vec![
            "Tab1".to_string(), "Tab2".to_string(), "Tab3".to_string(),
            "Tab4".to_string(), "Tab5".to_string(), "Tab6".to_string(),
            "Tab7".to_string(), "Tab8".to_string(), "Tab9".to_string(), "Tab10".to_string()
        ];
        
        // Should detect left arrow when scroll offset > 0
        let nav_action = calculate_tab_navigation_click(3, 0, 60, &tab_labels, 2, true);
        assert!(matches!(nav_action, Some(TabNavigationAction::ScrollLeft)), 
        ...
        let nav_action = calculate_tab_navigation_click(30, 0, 100, &few_tabs, 0, true);
        assert!(nav_action.is_none(), "Should return None when no scrolling needed");
    }
    ...
    fn test_tab_scrolling_with_active_tab_visibility() {
        ...
        assert_eq!(muxbox.get_active_tab_index(), 7, "Active tab should be index 7");
    }
}