...
fn test_custom_status_indicator_creation() {
    let indicator =
        StatusIndicator::new_custom("🎯".to_string(), Some("Target Acquired".to_string()));
...
fn test_status_indicator_chaining() {
    ...
    assert_eq!(
        indicator.process_info,
        Some("failed_process:5678".to_string())
    );
    assert_eq!(
        indicator.custom_text,
...
fn test_render_for_title_with_all_components() {
    ...
    assert_eq!(
        result,
        Some("⚡ [bash:1234] My Terminal Session".to_string())
    );
}
...
fn test_render_for_title_no_original_title() {
    let indicator =
        StatusIndicator::new(StatusType::PtyDead).with_process_info("crashed_app:9999".to_string());
...
fn test_custom_status_with_emoji_combinations() {
    ...
    let complex =
        StatusIndicator::new_custom("⚡🎯🚀".to_string(), Some("Multi-stage".to_string()));
