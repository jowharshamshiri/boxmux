...
mod tests {
    ...
    fn test_automatic_scrollbar_overflow_detection_logic() {
        ...
        assert!(content_overflows, "Content should overflow in small viewable area");
        assert!(content_height > viewable_height, "Content should overflow vertically");
    }
    ...
    fn test_focusable_panel_detection() {
        ...
        assert!(panel.next_focus_id.is_none(), "Panel without next_focus_id should not be focusable");
        
        // Focusable panel
        panel.next_focus_id = Some("next_panel".to_string());
        assert!(panel.next_focus_id.is_some(), "Panel with next_focus_id should be focusable");
    }
    ...
    fn test_overflow_behavior_modification() {
        ...
        let content = "Line 1\nLine 2\nLine 3\nLine 4\nLine 5\nLine 6\nLine 7\nLine 8\nLine 9\nLine 10\nLine 11\nLine 12\nLine 13\nLine 14\nLine 15";
        let mut overflow_behavior = panel.overflow_behavior.clone().unwrap_or("hidden".to_string());
