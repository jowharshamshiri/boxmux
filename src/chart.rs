...
pub fn generate_chart_with_panel_title(data: &[DataPoint], config: &ChartConfig, panel_title: Option<&str>) -> String {
    if data.is_empty() {
...
fn calculate_chart_layout(data: &[DataPoint], config: &ChartConfig, panel_title: Option<&str>) -> ChartLayout {
...
fn generate_bar_chart(data: &[DataPoint], config: &ChartConfig, layout: &ChartLayout, panel_title: Option<&str>) -> String {
    ...
    for (_i, point) in data.iter().enumerate() {
        ...
        for _ in 1..lines_per_bar {
            ...
            result.push_str(&format!("{} │{}\n", empty_label, " ".repeat(bar_width + value_str.len() + 1)));
        }
    }
...
fn generate_line_chart(data: &[DataPoint], config: &ChartConfig, layout: &ChartLayout, panel_title: Option<&str>) -> String {
    if data.len() < 2 {
    ...
    for (i, point) in data.iter().enumerate() {
        ...
        let y = if range > 0.0 {
            layout.chart_height - 1 - ((point.value - min_value) / range * (layout.chart_height - 1) as f64) as usize
        } else {
    ...
    if layout.x_label_height > 0 && !data.is_empty() {
        ...
        for (i, point) in data.iter().enumerate() {
            if i % step == 0 || i == data.len() - 1 {
                ...
                let spaces_before = x.saturating_sub(result.lines().last().unwrap_or("").len().saturating_sub(layout.y_label_width + 1));
...
fn generate_histogram(data: &[DataPoint], config: &ChartConfig, layout: &ChartLayout, panel_title: Option<&str>) -> String {
        ...
        if data.len() <= bins {
            ...
            for i in 0..labels_to_show {
                ...
                if end_pos <= layout.chart_width {
                    ...
                    let can_place = (start_pos..end_pos).all(|pos| {
                        pos >= line_chars.len() || line_chars[pos] == ' '
                    });
            ...
            let num_labels = if layout.chart_width > 80 { 8 } 
                           else if layout.chart_width > 60 { 6 } 
                           else if layout.chart_width > 40 { 4 } 
                           else { 3 };
            
            for i in 0..num_labels {
                let bin_idx = if num_labels == 1 { 0 } else { (i * (bins - 1)) / (num_labels - 1) };
...
mod tests {
    ...
    fn test_bar_chart_generation() {
        let data = vec![
            DataPoint { label: "Item1".to_string(), value: 10.0 },
            DataPoint { label: "Item2".to_string(), value: 25.0 },
        ];
