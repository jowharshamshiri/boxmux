...
pub fn render_table(data: &TableData, config: &TableConfig) -> String {
    ...
    // Apply sorting
    let sorted_rows = apply_sorting(&filtered_rows, &data.headers, &config.sort_column, config.sort_ascending);
    ...
    if config.show_headers {
        result.push_str(&render_header_row(&data.headers, &column_widths, &config.border_style, config.show_row_numbers));
    ...
    for (index, row) in paginated_rows.iter().enumerate() {
        ...
        result.push_str(&render_data_row(row, &column_widths, &config.border_style, config.show_row_numbers, index, is_highlighted, is_zebra));
    }
...
fn apply_sorting(rows: &[Vec<String>], headers: &[String], sort_column: &Option<String>, ascending: bool) -> Vec<Vec<String>> {
...
fn apply_pagination(rows: &[Vec<String>], pagination: &Option<TablePagination>) -> (Vec<Vec<String>>, Option<String>) {
...
fn calculate_column_widths(data: &TableData, config: &TableConfig, rows: &[Vec<String>]) -> Vec<usize> {
...
fn render_top_border(widths: &[usize], style: &TableBorderStyle) -> String {
    match style {
        ...
        TableBorderStyle::Custom { horizontal, vertical: _, top_left, top_right, bottom_left: _, bottom_right: _, cross } => {
...
fn render_bottom_border(widths: &[usize], style: &TableBorderStyle) -> String {
    match style {
        ...
        TableBorderStyle::Custom { horizontal, vertical: _, top_left: _, top_right: _, bottom_left, bottom_right, cross } => {
...
fn render_separator(widths: &[usize], style: &TableBorderStyle) -> String {
    match style {
        ...
        TableBorderStyle::Custom { horizontal, vertical, top_left: _, top_right: _, bottom_left: _, bottom_right: _, cross } => {
...
/// Render header row
fn render_header_row(headers: &[String], widths: &[usize], style: &TableBorderStyle, show_row_numbers: bool) -> String {
