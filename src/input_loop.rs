...
create_runnable!(
    ...
    |inner: &mut RunnableImpl, state: AppContext, messages: Vec<Message>| -> bool {
        // Processing block
        info!("Processing in InputLoop with state: {:?} and messages: {:?}", state, messages);
		...
		for c in stdin.keys() {
			match c.unwrap() {
				...
				Key::Char('\t') => {
					inner.send_message(Message::NextPanel("Panel3".to_string()));
				}
				Key::BackTab => {
					inner.send_message(Message::PreviousPanel("Panel3".to_string()));
				}
				Key::Down => {
