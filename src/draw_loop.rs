...
use crate::{apply_buffer_if_changed, screen_height, screen_width, AppContext, Layout, Panel, ScreenBuffer};
use log::{error, info};
...
use crate::utils::{fill_panel,draw_panel as util_draw_panel, screen_bounds};
use uuid::Uuid;



lazy_static! {
    static ref GLOBAL_SCREEN: Mutex<Option<AlternateScreen<RawTerminal<Stdout>>>> = Mutex::new(None);
...
create_runnable!(
    ...
    |inner: &mut RunnableImpl, state: AppContext, messages: Vec<Message>| -> bool {
        ...
        if let (Some(ref mut screen), Some(ref mut buffer)) = (&mut *global_screen, &mut *global_buffer) {
    ...
    |inner: &mut RunnableImpl, state: AppContext, messages: Vec<Message>| -> bool {
            ...
            for message in &messages {
                match message {
                    ...
                    Message::NextPanel(panel_id) => {
                        for layout in &mut state_unwrapped.app.layouts {
                    ...
                    Message::RedrawPanel(panel_id) => {
                        ...
                        if let Some(found_panel) = panel {
                            ...
                            draw_panel(&state_unwrapped, &found_panel.get_parent_layout(&state_unwrapped).unwrap(), &found_panel, &mut new_buffer);
                            apply_buffer_if_changed(buffer, &new_buffer, screen);
                        }
                    }
                    Message::RedrawApp => {
		...
		log::info!("DrawLoop finished processing messages");
        should_continue
    }
);
...
pub fn draw_app(
	...
	for layout in &app.layouts {
		draw_layout(app_context, layout, buffer);
	}
...
pub fn draw_panel(
	...
	let bg_color = panel.calc_bg_color(app_context).to_string();
	let parent_bg_color = if panel_parent.is_none() {
		"default".to_string()
	} else {
		panel_parent
			.unwrap()
			.calc_bg_color(app_context)
			.to_string()
	};
