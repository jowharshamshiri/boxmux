...
impl BoxMuxSocketHandler {
    pub async fn register_handlers(&self, server: &mut JanusServer) -> Result<(), JanusError> {
        ...
        server.register_handler("replace-panel-content", move |cmd: SocketCommand| {
            let args = cmd.args.unwrap_or_default();
            let panel_id = args.get("panel_id")
                .and_then(|v| v.as_str())
                .ok_or_else(|| JanusError::ValidationError("Missing required argument: panel_id".to_string()))?;
            let success = args.get("success")
                .and_then(|v| v.as_bool())
                .ok_or_else(|| JanusError::ValidationError("Missing required argument: success".to_string()))?;
            let content = args.get("content")
                .and_then(|v| v.as_str())
                .ok_or_else(|| JanusError::ValidationError("Missing required argument: content".to_string()))?;
        ...
        let message_sender_clone = self.message_sender.clone();
        let sender_uuid_clone = self.sender_uuid;
        server.register_handler("replace-panel-script", move |cmd: SocketCommand| {
            let args = cmd.args.unwrap_or_default();
            let panel_id = args.get("panel_id")
                .and_then(|v| v.as_str())
                .ok_or_else(|| JanusError::ValidationError("Missing required argument: panel_id".to_string()))?;
            let script_array = args.get("script")
                ...
                .ok_or_else(|| JanusError::ValidationError("Missing required argument: script".to_string()))?;
        ...
        let message_sender_clone = self.message_sender.clone();
        let sender_uuid_clone = self.sender_uuid;
        server.register_handler("stop-panel-refresh", move |cmd: SocketCommand| {
            let args = cmd.args.unwrap_or_default();
            let panel_id = args.get("panel_id")
                .and_then(|v| v.as_str())
                .ok_or_else(|| JanusError::ValidationError("Missing required argument: panel_id".to_string()))?;
        ...
        let message_sender_clone = self.message_sender.clone();
        let sender_uuid_clone = self.sender_uuid;
        server.register_handler("start-panel-refresh", move |cmd: SocketCommand| {
            let args = cmd.args.unwrap_or_default();
            let panel_id = args.get("panel_id")
                .and_then(|v| v.as_str())
                .ok_or_else(|| JanusError::ValidationError("Missing required argument: panel_id".to_string()))?;
        ...
        let message_sender_clone = self.message_sender.clone();
        let sender_uuid_clone = self.sender_uuid;
        server.register_handler("switch-active-layout", move |cmd: SocketCommand| {
            let args = cmd.args.unwrap_or_default();
            let layout_id = args.get("layout_id")
                .and_then(|v| v.as_str())
                .ok_or_else(|| JanusError::ValidationError("Missing required argument: layout_id".to_string()))?;
        ...
        let message_sender_clone = self.message_sender.clone();
        let sender_uuid_clone = self.sender_uuid;
        server.register_handler("replace-panel", move |cmd: SocketCommand| {
            let args = cmd.args.unwrap_or_default();
            let panel_id = args.get("panel_id")
                .and_then(|v| v.as_str())
                .ok_or_else(|| JanusError::ValidationError("Missing required argument: panel_id".to_string()))?;
            let new_panel_json = args.get("new_panel")
                .ok_or_else(|| JanusError::ValidationError("Missing required argument: new_panel".to_string()))?;
            ...
            let new_panel: crate::Panel = serde_json::from_value(new_panel_json.clone())
                .map_err(|e| JanusError::ValidationError(format!("Invalid panel definition: {}", e)))?;
        ...
        let message_sender_clone = self.message_sender.clone();
        let sender_uuid_clone = self.sender_uuid;
        server.register_handler("add-panel", move |cmd: SocketCommand| {
            let args = cmd.args.unwrap_or_default();
            let layout_id = args.get("layout_id")
                .and_then(|v| v.as_str())
                .ok_or_else(|| JanusError::ValidationError("Missing required argument: layout_id".to_string()))?;
            let panel_json = args.get("panel")
                .ok_or_else(|| JanusError::ValidationError("Missing required argument: panel".to_string()))?;
        ...
        let message_sender_clone = self.message_sender.clone();
        let sender_uuid_clone = self.sender_uuid;
        server.register_handler("remove-panel", move |cmd: SocketCommand| {
            let args = cmd.args.unwrap_or_default();
            let panel_id = args.get("panel_id")
                .and_then(|v| v.as_str())
                .ok_or_else(|| JanusError::ValidationError("Missing required argument: panel_id".to_string()))?;
...
fn handle_socket_function(
    ...
    if let Err(e) = message_sender.send((sender_uuid, boxmux_message)) {
        ...
        return Err(JanusError::ValidationError(format!("Failed to process command: {}", e)));
    }
...
mod tests {
    ...
    fn test_handle_socket_function_send_error() {
        ...
        if let Err(JanusError::ValidationError(msg)) = result {
            assert!(msg.contains("Failed to process command"));
        } else {
    ...
    fn test_socket_command_missing_arguments() {
        ...
        let result = (move |cmd: SocketCommand| {
            ...
            Ok::<serde_json::Value, JanusError>(json!({"status": "success"}))
        })(socket_cmd);
    ...
    fn test_socket_command_panel_json_parsing() {
        ...
        match result {
            ...
            Err(JanusError::ValidationError(msg)) => {
                // Expected if Panel deserialization fails with test JSON
                assert!(msg.contains("Invalid panel definition"));
            }
