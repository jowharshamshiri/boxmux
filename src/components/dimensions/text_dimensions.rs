    ...
    pub fn layout_text(&self, text: &str) -> TextLayout {
        ...
        let (start_x, start_y) = match self.alignment {
            ...
            TextAlignment::Center => {
                let x = self.bounds.x1 + (self.available_width().saturating_sub(text_width)) / 2;
                let y = self.bounds.y1 + (self.available_height().saturating_sub(text_height)) / 2;
                (x, y)
            }
            TextAlignment::Right => {
    ...
    pub fn calculate_line_positions(&self, layout: &TextLayout) -> Vec<LinePosition> {
        ...
        for (line_index, line) in layout.lines.iter().enumerate() {
            ...
            let x = match layout.alignment {
                ...
                TextAlignment::Right => self.bounds.x2.saturating_sub(line_width),
...
mod tests {
    ...
    fn test_text_truncation() {
        ...
        let text_dims = TextDimensions::new(bounds).with_wrap_behavior(WrapBehavior::Truncate);
    ...
    fn test_center_alignment() {
        ...
        let text_dims = TextDimensions::new(bounds).with_alignment(TextAlignment::Center);
