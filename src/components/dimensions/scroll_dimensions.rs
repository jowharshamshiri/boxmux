...
impl ScrollDimensions {
    ...
    pub fn set_scroll_position(&mut self, horizontal: f64, vertical: f64) {
        self.scroll_position = (horizontal.clamp(0.0, 100.0), vertical.clamp(0.0, 100.0));
    }
    ...
    pub fn hits_knob(&self, x: usize, y: usize, orientation: Orientation) -> bool {
        ...
        match orientation {
            Orientation::Vertical => x == knob.x && y >= knob.y_start && y < knob.y_end,
            Orientation::Horizontal => y == knob.y_start && x >= knob.x && x < (knob.x + knob.size),
        }
    }
...
mod tests {
    ...
    fn test_pixel_to_scroll_percent() {
        let bounds = Bounds::new(0, 0, 10, 10);
        let scroll_dims = ScrollDimensions::new((20, 20), (10, 10), (0.0, 0.0), bounds);
