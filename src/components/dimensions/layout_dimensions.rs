...
impl LayoutDimensions {
        ...
        let x1 = if input.x1.is_empty() {
            ...
            base.x1
                + input
                    .x1
                    .parse::<usize>()
                    .map_err(|_| LayoutError::InvalidCoordinate(input.x1.clone()))?
        };
        ...
        let y1 = if input.y1.is_empty() {
            ...
            base.y1
                + input
                    .y1
                    .parse::<usize>()
                    .map_err(|_| LayoutError::InvalidCoordinate(input.y1.clone()))?
        };
        ...
        let x2 = if input.x2.is_empty() {
            ...
            base.x1
                + input
                    .x2
                    .parse::<usize>()
                    .map_err(|_| LayoutError::InvalidCoordinate(input.x2.clone()))?
        };
        ...
        let y2 = if input.y2.is_empty() {
            ...
            base.y1
                + input
                    .y2
                    .parse::<usize>()
                    .map_err(|_| LayoutError::InvalidCoordinate(input.y2.clone()))?
        };
    ...
    pub fn distribute_space(&self, percentages: &[f64]) -> Result<Vec<usize>, LayoutError> {
        ...
        if total_percent > 100.01 {
            ...
            return Err(LayoutError::PercentageOverflow {
                total: total_percent,
            });
        }
    ...
    pub fn calculate_minimum_space(&self) -> (usize, usize) {
        ...
        match &self.layout_strategy {
            ...
            LayoutStrategy::Flex { .. } | LayoutStrategy::Stack => {
                ...
                (
                    min_child_size.0,
                    min_child_size.1 * self.child_constraints.len(),
                )
            }
        }
    }
...
mod tests {
    ...
    fn test_grid_layout() {
        ...
        layout = layout.with_strategy(LayoutStrategy::Grid {
            columns: 2,
            rows: 2,
        });
