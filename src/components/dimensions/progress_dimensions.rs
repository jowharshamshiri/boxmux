...
impl ProgressDimensions {
    ...
    pub fn map_pixel_to_value(&self, pixel: usize) -> f64 {
        ...
        let relative_pixel = match self.orientation {
            Orientation::Horizontal => pixel.saturating_sub(self.bounds.x1),
            Orientation::Vertical => {
    ...
    pub fn calculate_bars(&self, data: &[f64]) -> Vec<ChartBar> {
        ...
        for (i, &value) in data.iter().enumerate() {
            ...
            let bar_top_y = self
                .bounds
                .y2
                .saturating_sub(bar_height_pixels.saturating_sub(1));
    ...
    pub fn calculate_sparkline(&self, data: &[f64]) -> Vec<SparklinePoint> {
        ...
        if data.len() <= width {
            ...
            let x_step = if data.len() == 1 {
                ...
                (width - 1) as f64 / (data.len() - 1) as f64
            };
            ...
            if data.len() > 1 {
                for x_pos in 0..width {
                    ...
                    if needs_point {
                        ...
                        let data_pos =
                            (x_pos as f64 / (width - 1) as f64) * (data.len() - 1) as f64;
    ...
    pub fn validate(&self) -> Result<(), ProgressDimensionError> {
        ...
        let (min_val, max_val) = self.data_range;
        if min_val >= max_val {
            return Err(ProgressDimensionError::InvalidDataRange {
                min: min_val,
                max: max_val,
            });
        }
...
mod tests {
    ...
    fn test_bar_chart_calculation() {
        ...
        let max_bar = bars
            .iter()
            .max_by(|a, b| a.height_pixels.cmp(&b.height_pixels))
            .unwrap();
