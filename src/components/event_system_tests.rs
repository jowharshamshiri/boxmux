...
mod tests {
    ...
    fn test_content_event_creation() {
        ...
        assert_eq!(key_event.key_info().unwrap().modifiers, vec![KeyModifier::Ctrl]);
    }
    ...
    fn test_content_event_helper_methods() {
        ...
        assert_eq!(scroll_event.scroll_info().unwrap().direction, ScrollDirection::Down);
    ...
    fn test_choice_content_event_handling() {
        ...
        let key_event = ContentEvent::new_keypress("ArrowDown".to_string(), vec![], Some("choice_0".to_string()));
    ...
    fn test_text_content_event_handling() {
        ...
        let copy_event = ContentEvent::new_keypress(
            "c".to_string(), 
            vec![KeyModifier::Ctrl], 
            None
        );
    ...
    fn test_event_backward_compatibility() {
        ...
        // Test that deprecated handle_click still works
        let result = choice_content.handle_click("choice_0");
        assert!(result);
        
        let invalid_result = choice_content.handle_click("choice_5");
        assert!(!invalid_result);
        
        // Test with text content
        let mut text_content = TextContent::new("test", &None, &None);
        let result = text_content.handle_click("any_zone");
        assert!(!result); // Text doesn't handle clicks
    }
    ...
    fn test_scroll_directions() {
        ...
        assert_eq!(down_scroll.scroll_info().unwrap().direction, ScrollDirection::Down);
        
        let up_scroll = ContentEvent::new_scroll(ScrollDirection::Up, 3, None);
        assert_eq!(up_scroll.scroll_info().unwrap().direction, ScrollDirection::Up);
        
        let left_scroll = ContentEvent::new_scroll(ScrollDirection::Left, 2, None);
        assert_eq!(left_scroll.scroll_info().unwrap().direction, ScrollDirection::Left);
        
        let right_scroll = ContentEvent::new_scroll(ScrollDirection::Right, 1, None);
        assert_eq!(right_scroll.scroll_info().unwrap().direction, ScrollDirection::Right);
    }

    #[test]
    fn test_mouse_move_events() {
        // Test basic mouse move
        let move_event = ContentEvent::new_mouse_move(Some((5, 5)), (10, 8), Some("text_area".to_string()));
        ...
        // Test mouse drag
        let drag_event = ContentEvent::new_mouse_drag((5, 5), (15, 10), MouseButton::Left, Some("choice_1".to_string()));
    ...
    fn test_hover_events() {
        ...
        assert_eq!(hover_info.hover_duration.unwrap(), Duration::from_millis(1500));
    }
    ...
    fn test_extended_choice_event_handling() {
        ...
        let mouse_move = ContentEvent::new_mouse_move(Some((5, 0)), (6, 0), Some("choice_0".to_string()));
        let result = choice_content.handle_event(&mouse_move);
        assert_eq!(result, EventResult::HandledContinue);
        
        // Test mouse drag
        let mouse_drag = ContentEvent::new_mouse_drag((5, 0), (10, 0), MouseButton::Left, Some("choice_0".to_string()));
